...
class LabeledTriosSeries(KnownF):
    ...
    def __init__(self, series=[],series_names=[]):
        ...
        for i in range(len(series)):
            ...
            y=[[item[1],item[2]] for item in series[i]]
            ...
            for j in range(len(series[i])):
                if j>= len(mins):
                    ...
                    self.proms[j]=([new_proms_x,new_proms_y,new_proms_w],1+self.proms[j][1])
        self.count=len(self.elements)
        self.elements["Maxs_Series"]=maxs
        self.elements["Mins_Series"]=mins
        self.elements["Meds_Series"]=[[i[0],i[1]/j,i[2]/j] for i,j in self.proms]
    def update_min_max_meddian(self,kf_mins,kf_maxs,kf_proms):
        ...
        for i in range(len(kf_mins)):
            if i>=len(mins):
                ...
                proms[i]=([new_proms_x,new_proms_y,new_proms_w],proms[i][1]+kf_proms[i][1])
        # self.elements["Maxs_Series"]=maxs
        # self.elements["Mins_Series"]=mins
        self.elements["Meds_Series"]=[[i[0],i[1]/j,i[2]/j] for i,j in proms]